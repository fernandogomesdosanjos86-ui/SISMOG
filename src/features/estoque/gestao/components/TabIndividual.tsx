...
const TabIndividual: React.FC<TabIndividualProps> = ({ onRefresh, deleteMov }) => {
    ...
    const loadResumo = async () => {
        setIsLoading(true);
        try {
            const data = await estoqueGestaoService.getResumoPorFuncionario();
            setResumo(data);
        } catch (e) {
            console.error(e);
        } finally {
    ...
    const columns = [
        {
            key: 'nome', header: 'Funcionário', render: (r: ResumoFuncionario) => (
                <span className="font-medium text-gray-900">{r.nome} - {r.empresa}</span>
            )
        },
        {
            key: 'qtd', header: 'Qtd Itens em Mãos', render: (r: ResumoFuncionario) => (
                <span className="font-bold text-blue-700">{r.qtd}</span>
            )
        },
    ];
    ...
    const renderCard = (r: ResumoFuncionario) => (
        <div className="flex justify-between items-center">
            ...
            <span className="text-lg font-bold text-blue-700">{r.qtd}</span>
        </div>
    );
        ...
        <div className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg">
                <div className="grid grid-cols-3 gap-4 text-center">
                    ...
                    <div>
                        <p className="text-xs text-gray-500 uppercase">Itens em Mãos</p>
                        <p className="text-xl font-bold text-blue-700">{qtdTotal}</p>
                    </div>
                </div>
            </div>
